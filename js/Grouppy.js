...
//     displayFileContent(memberFileDetails.name);
//   };
...
//     displayFileContent(leaderFileDetails.name);
//   };
...
//   // // Distribute remaining members among the groups
//   // let memberIndex = 0;
//   // for (let i = 0; i < totalGroups; i++) {
//   //   while (groups[i].length < 4 && memberIndex < shuffledMembers.length) {
//   //     groups[i].push(shuffledMembers[memberIndex]);
//   //     memberIndex++;
//   //   }
//   // }
...
//     let worksheet = XLSX.utils.aoa_to_sheet(group);
//     XLSX.utils.book_append_sheet(workbook, worksheet, `Group ${index + 1}`);
//   });

//   XLSX.writeFile(workbook, "groups_with_leaders.xlsx");
// }
...
// // Remaining code for file handling, DOM manipulation, and event listeners...
...
//     // Flatten the groups array into a single array
//     let allGroups = [];
//     groups.forEach((group, index) => {
//         allGroups.push([`Group ${index + 1}`]); // Add a row for the group name
//         allGroups = allGroups.concat(group); // Add the group members
//         allGroups.push([]); // Add an empty row between groups
//     });
...
//             fill: { fgColor: { rgb: "FFFF00" } }, // Yellow for leader
//           };
//         } else {
//           worksheet[cellAddress].s = {
